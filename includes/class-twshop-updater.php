 ...
 * v1.32.0/v1.33.0 用同一套機制驗證過可行，這裡是移植而非重新設計。
 *
 * repo 是公開的，`TWSHOP_GITHUB_TOKEN` 常數純選填：沒設定一樣能正常檢查更新（公開 repo
 * 的內容/Release API 本來就不需要驗證身份），設定了則用來呼叫 GitHub API 拉高請求頻率上限。
 *
 * 排除機制（見 CLAUDE.md「自架更新通道」一節）：
 *   1. `wp-config.php` 定義 `TWSHOP_DISABLE_UPDATES`（任何 truthy 值）——完全在該網站本地
 *      判斷，不對外發任何請求，最後一道保險。
 *   2. repo 裡的 `update-config.json`（`disabled_site_hashes`）——集中管理，存的是網域的
 *      SHA-256 雜湊值，不是明文網域（repo 公開，明文網域等於公開告訴大家「這幾個網站在用
 *      這個外掛」）。每個網站拿自己的網域算雜湊值來比對，不需要額外設定「我是誰」。
 * 兩者任一成立就跳過整個更新檢查器的註冊。
 */
...
class TWSHOP_Updater {
    ...
    public static function init() {
        if ( defined( 'TWSHOP_DISABLE_UPDATES' ) && TWSHOP_DISABLE_UPDATES ) {
            return;
        }
        ...
        require_once TWSHOP_PLUGIN_DIR . 'vendor/plugin-update-checker/plugin-update-checker.php';
        ...
        if ( defined( 'TWSHOP_GITHUB_TOKEN' ) && '' !== TWSHOP_GITHUB_TOKEN ) {
            $update_checker->setAuthentication( TWSHOP_GITHUB_TOKEN );
        }
     ...
     * 請用上面的 TWSHOP_DISABLE_UPDATES 常數當雙保險。
     */
    ...
    private static function fetch_disabled_site_hashes() {
        $args = array(
            'headers' => array(
                ...
                'User-Agent' => 'twshop-update-checker',
            ),
            'timeout' => 8,
        );
        ...
        // 一樣讀得到內容。
        if ( defined( 'TWSHOP_GITHUB_TOKEN' ) && '' !== TWSHOP_GITHUB_TOKEN ) {
            $args['headers']['Authorization'] = 'token ' . TWSHOP_GITHUB_TOKEN;
        }
